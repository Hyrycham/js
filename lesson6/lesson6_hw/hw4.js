...
function keyGet (userObj,ppp){
 ...
 if (typeof userObj[key] === 'object' && !Array.isArray(userObj[key])){
      
  let divUserAttr=document.createElement('div');
 divUserAttr.style.padding='5px 5px 5px 5px';
 divUserAttr.style.margin='5px 5px 5px 5px';
 divUserAttr.style.backgroundColor='grey';
 divUserAttr.style.borderBottom='black 1px solid';
...
divUserAttr.innerHTML=`${count}`
 ...
 {let divUserAttr=document.createElement('div');
 divUserAttr.style.padding='3px 3px 0px 3px';
 divUserAttr.style.backgroundColor='silver';
 divUserAttr.style.borderBottom='black 1px solid';

ppp.appendChild(divUserAttr);
  count=`${key}:${userObj[key]} <br>`
divUserAttr.innerHTML=`${count}`
}  
} 
  }
